...
from utils import get_dataset_embeddings, make_directory
from matplotlib import pyplot as plt
import os
import pickle

PLOTS_PATH = make_directory("../plots/")
RESULTS_PATH = make_directory("../results/")
...
def query_AP(is_target, n_groundtruths, k):
    normalizer = min(n_groundtruths, k) # TODO: What is the correct definition?
...
def evaluation(model, query_loader, gallery_loader):
    ...
    print("K=1 Recall:{} HIT:{} mAP:{}".format(k_1["recall"], k_1["hit"], k_1["map"]))
    print("K=5 Recall:{} HIT:{} mAP:{}".format(k_5["recall"], k_5["hit"], k_5["map"]))
    print("K=50 Recall:{} HIT:{} mAP:{}".format(k_50["recall"], k_50["hit"], k_50["map"]))
    print("K=100 Recall:{} HIT:{} mAP:{}".format(k_100["recall"], k_100["hit"], k_100["map"]))
    print("K=|gallery| Recall:{} HIT:{} mAP:{}".format(k_gallery["recall"], k_gallery["hit"], k_gallery["map"]))
...
def write_results(metrics, experiment_name):
    for metric in metrics.keys():
        with open(os.path.join(RESULTS_PATH, experiment_name + "_" + metric)+'.plk', 'wb') as f:
